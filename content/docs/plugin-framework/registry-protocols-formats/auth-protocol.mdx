...
The Bluelink CLI supports authentication via the `bluelink plugins login {host}` command, where host is expected to be a host that contains a service discovery document as per the [Service Discovery Protocol](/docs/plugin-framework/service-discovery-protocol).

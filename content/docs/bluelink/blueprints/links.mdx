...
## Excluding Resources from Link Selection

When multiple resources share the same labels, the `linkSelector` will match all of them by default.
In cases where you need to exclude specific resources from being linked, you can use the `exclude` field.

The `exclude` field takes a list of resource names that should be skipped during link selection,
even if they match the `byLabel` criteria:
  ...
  orderProcessor:
    ...
    linkSelector:
      byLabel:
        app: orderApi
      exclude:
        - testOrderTable  # Won't link to testOrderTable even though it has app: orderApi label
    spec:
...
- Excluding test or staging resources from production workloads
- Preventing specific resources from being linked when labels are shared across multiple resources
- Fine-tuning link relationships without creating additional label dimensions

For more details on the `exclude` field, see the [linkSelectorDefinitions](/docs/bluelink/blueprints/specification#linkselectordefinitions) specification.

## Activating Links from References

A `linkSelector` is one of two ways a link is activated. The other is a reference: for some resource fields, placing a reference to another resource in that field activates the link to that resource automatically, with no `linkSelector` required.

This applies to resource types that hold a related resource's identifier directly in their own spec. A common example is an Amazon EventBridge rule, which lists its targets inline and requires each target to carry the ARN of the resource it invokes. Filling that field with a reference both supplies the required configuration and tells the provider which resource to link to, so adding a `linkSelector` for the same pair would only duplicate the relationship.

Previously you might have written both a `linkSelector` on the rule and a reference to the function for the same pair:
...
resources:
  ...
  ordersRule:
    ...
    spec:
      ...
      targets:
        - id: ordersTarget
          arn: ${ordersFunction.spec.arn}
```

The reference in `targets[].arn` already activates the rule-to-function link, so the `linkSelector` (and the `app: orders` label that existed only to satisfy it) can be removed:
...
- Whether a reference activates a link depends on the field it is placed in, not on which property of the target it points at. Which fields behave this way is documented with each resource type.
- A link must exist between the two resource types for activation to happen. If no link is available for the pair, the reference is treated as a plain value.
- If both a `linkSelector` and an activating reference are present for the same pair, the link is still activated only once.

<Callout title="Note" type="info">
Not every reference activates a link. A reference used purely for its value, such as a resource's computed region or a numeric attribute injected into an environment variable or a tag, does not activate anything. Only references placed in fields that a resource type treats as link targets activate a link.
</Callout>

## Link Configuration with Annotations
...
The same way resource state can be accessed through the use of [references](/docs/bluelink/blueprints/specification#references--substitutions), the state of a link should be accessible with the `link(resourceA, resourceB)` function as per the [specification](/docs/blueprint-framework/core-functions#link). This is useful for accesing state related to annotations used to configure a link.

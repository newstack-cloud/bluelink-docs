---
title: Introduction
description: Introduction to the Plugin Framework
---

The Plugin Framework allows you to extend the functionality of the Deploy Engine with blueprint resources, data sources, functions and more. Plugins can either provide elements that can be used in blueprints (e.g. resource types) or apply transformations to blueprints; the former is considered a `Provider` and the latter a `Transformer`, both concepts are defined in the [Blueprint Framework](/docs/blueprint-framework).

## Plugin System Overview

The plugin system is made up of three main components, the [Plugin Service](/docs/plugin-framework/plugin-system#plugin-service), [Provider](/docs/plugin-framework/provider) plugins and [Transformer](/docs/plugin-framework/transformer) plugins.
...
[Learn more about the Plugin System](/docs/plugin-framework/plugin-system)
...
[Take me to the SDK Documentation](/docs/plugin-framework/sdk)
...
An example for when you might want to host your own registry is when you have private plugins that you don't want to share with the public but want to leverage the Bluelink CLI to manage them for an instance of the Deploy Engine.

[Start with the Registry Protocol](/docs/plugin-framework/registry-protocols-formats/registry-protocol)

...
Out of the box, the Deploy Engine discovers plugins by looking for executables under the `$HOME/.bluelink/deploy-engine/plugins/bin` directory. The `bin` directory is expected to contain a directory for each type of plugin. Provider plugins should be in the `providers` directory, and transformer plugins should be in the `transformers` directory.

Each plugin is expected to be in a subdirectory following the ID format of `{hostname/}?{namespace}/{plugin}`. 
For example, a plugin for the `bluelink/aws` provider would be located at `$HOME/.bluelink/deploy-engine/plugins/bin/providers/bluelink/aws`. Another example, would be that the plugin with the ID `registry.customhost.com/bluelink/azure` would be expected to be located in `$HOME/.bluelink/deploy-engine/plugins/bin/providers/registry.customhost.com/bluelink/azure`. 
...
In each plugin directory, there should be a folder for each version of the plugin. The version folder should be named with the version number of the plugin. For example, the plugin version `1.0.0` would be located at `$HOME/.bluelink/deploy-engine/plugins/bin/providers/bluelink/aws/1.0.0`.

Each plugin executable is expected to be a file named `plugin` with no file extension.
An example of a full path to the plugin executable file would be `$HOME/.bluelink/deploy-engine/plugins/bin/providers/bluelink/aws/1.0.0/plugin`.

For a standard install of the Deploy Engine, the `$HOME/.bluelink/deploy-engine/plugins/bin` directory is created for you and populated with the core plugins that enable the Bluelink resource types.

You can specify a custom directory for the Deploy Engine to look for plugins by appending a new directory to the `BLUELINK_DEPLOY_ENGINE_PLUGIN_PATH` environment variable. This environment variable is a colon-separated list of directories that the Deploy Engine will search for plugins in. This environment variable will be set as a part of the standard Bluelink installation with a default value of `$HOME/.bluelink/deploy-engine/plugins/bin`. If, for some reason, this environment variable is not set, the Deploy Engine will default to the `$HOME/.bluelink/deploy-engine/plugins/bin` directory.

An example for a custom plugin path would be to set the `$HOME/.bluelink/deploy-engine/plugins/bin:/path/to/custom/plugins`.

...
These commands are useful for sharing deploy engine state between different environments when adopting a local deployment strategy.
...
<Callout title="Note" type="info">
The state commands can only be used when the deploy engine is running on the same machine as the CLI.
...
```bash
bluelink state import --file ./deployment-state.tar.gz
```
...
bluelink state import --instances --file s3://bucket-name/path/to/instances-export.json
```

#### `--instances`

**Environment Variable**: `BLUELINK_CLI_STATE_IMPORT_INSTANCES`

**Configuration Key**: `stateImportInstances`
...
```bash
bluelink state export --file ./deployment-state.tar.gz
```
...
<Callout title="Note" type="info">
Cleanup operations run asynchronously in the background. The command returns immediately after
triggering the cleanup process. The actual cleanup may take several minutes to complete depending
on the amount of data to be removed.
</Callout>

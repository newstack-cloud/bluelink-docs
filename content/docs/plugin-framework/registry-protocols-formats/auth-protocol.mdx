...
description: The protocol for authenticating with a registry service
---
...
The Bluelink CLI supports authentication via the `bluelink plugins login {host}` command, where host is expected to be a host that contains a service discovery document as per the [Service Discovery Protocol](/docs/plugin-framework/registry-protocols-formats/service-discovery-protocol).

The `bluelink plugins login {host}` command will prompt the user to enter the necessary configuration to be stored in the `$HOME/.bluelink/clients/plugins.auth.json` file (`%LOCALAPPDATA%\NewStack\Bluelink\clients\plugins.auth.json` on Windows) or will take them through the OAuth 2 authorization code flow to obtain an access token depending on the configuration provided in the service discovery document.
...
On the client side (e.g. environment running the Bluelink CLI), the API key must be set in the `$HOME/.bluelink/clients/plugins.auth.json` (Linux or macOS) or `%LOCALAPPDATA%\NewStack\Bluelink\clients\plugins.auth.json` (Windows) file with a mapping of the host to the API key as follows:

```json title="$HOME/.bluelink/clients/plugins.auth.json"
{
  "registry.bluelink.dev": {
    "apiKey": "{api-key}"
  }
}
```
...
On the client side (e.g. environment running the Bluelink CLI), the OAuth 2 client credentials must be set in the `plugins.auth.json` file with a mapping of the host to the client credentials as follows:
...
- `pkce` (Optional) - A boolean indicating whether to use PKCE (Proof Key for Code Exchange) for the authorization code flow. The default is `false`.

---
title: Specification
description: The blueprint language specification
---
...
This is a specification of resources that can be deployed such as [Celerity](https://celerityframework.io) application resources.
...
A blueprint can either be in the [Blueprint Language](/docs/bluelink/blueprints/language), JWCC ([JSON with Commas and Comments](https://nigeltao.github.io/blog/2021/json-with-commas-comments.html)) or YAML format.
...
For the v2025-11-02 version of the specification, the only valid value is `2025-11-02`.
...
Exports can not carry out any computation for the `field` property. The canonical schema treats `field` as a pure path, so function calls and computed expressions are not accepted in any of the three formats: in YAML and JWCC, the `field` value is a plain string path and cannot contain a `${..}` substitution; in the blueprint language, `field` accepts a bare reference such as `resources.s3Bucket.spec.name` but not a function call or operator expression. Functions are otherwise available everywhere expressions are: inside `${..}` in YAML and JWCC, and as bare expressions in the blueprint language.
...
Blueprint Language Static Object

```blueprintlang
value logsPolicyDoc: object {
    value = {
...
Blueprint Language Static Array
...
Blueprint Language with multiple filters
...
data network: aws/vpc {
    ...
    filter "tags" has key variables.environment
    filter "region" in variables.awsRegions
...
Blueprint Language exporting all fields
...
A custom variable type must be of the format `{provider}/{type}` or `{provider}/{service}/{type}`.
...
<Callout title="Tip" type="info">
Only scalar values are supported for variables. To pass in a complex structure such as an array or object, encode it as a JSON string and decode it with the `jsondecode` function. In YAML and JWCC the call is written inside a `${..}` substitution; in the blueprint language it is written as a bare expression.

For example, calling a provider's custom function that takes a specific object structure as input would look like the following.
...
```
custom_provider_function(jsondecode(variables.deploymentConfig))
```

Where `variables.deploymentConfig` would be a serialised JSON string defined in the blueprint language:

```blueprintlang
variable deploymentConfig: string {
...
<FieldHeading fieldName="allowedValues" />
...
<FieldHeading fieldName="value (required)" />
...
<FieldHeading fieldName="metadata" />
...
<FieldHeading fieldName="filter (required)" />
...
<FieldHeading fieldName="exports (required)" />
...
<FieldHeading fieldName="displayName" />
...
<FieldHeading fieldName="annotations" />
...
The value of an annotation must be a scalar or can be a string containing `${..}` substitutions that resolve to a scalar value.
...
<FieldHeading fieldName="field (required)" />
...
<FieldHeading fieldName="operator (required)" />
...
<FieldHeading fieldName="search (required)" />
...
<Callout title="Tip" type="info">
As only primitive values and primitive arrays are supported, data source developers should model complex data structures using field names such as `loggingConfig.applicationLogLevel` to represent nested fields in the data source object, or populate exported data source fields with JSON-encoded strings that can be parsed in blueprints using the [`jsondecode`](/docs/blueprint/core-functions#jsondecode) function. In YAML and JWCC, the call is written inside a `${..}` substitution; in the blueprint language, it is a bare expression on the right-hand side of a field assignment.

When using names with dot notation to represent nested fields, the bracket accessor is required. In YAML and JWCC the reference is wrapped in `${..}`:
`${datasources.myFunction["loggingConfig.applicationLogLevel"]}`

In the blueprint language the same reference is written bare:
`datasources.myFunction["loggingConfig.applicationLogLevel"]`
...
<FieldHeading fieldName="aliasFor" />
...
<FieldHeading fieldName="dependsOn" />
...
<FieldHeading fieldName="condition" />
...
The condition can be defined in 2 ways. 1, as a boolean expression that evaluates to `true` or `false`; 2, as a condition object that allows conditions to be combined with `and`, `or` and `not` operators. In YAML and JWCC, the boolean expression is wrapped in a `${..}` substitution (for example, `${eq(variables.deploymentTarget, "serverless")}`); in the blueprint language, the same condition is written as a bare expression using the infix operators (`variables.deploymentTarget == "serverless"`). Both lower to the same canonical schema.

<Callout title="Tip" type="info">
In YAML and JWCC, more complex logical expressions are built inside `${..}` with the `and`, `or` and `not` functions. The blueprint language exposes the same operators as infix `&&`, `||` and prefix `!`.
...
```
variables.deploymentTarget == "container" || variables.deploymentTarget == "cloudFunctions"
```
</Callout>
...
<FieldHeading fieldName="each" />

Provides a way to create multiple resources based on a list of values that can be referenced in the blueprint. The current item in the list is referenced as `elem`, and the current index as `i`. In YAML and JWCC, both are read inside a `${..}` substitution (`${elem.bucketName}`, `${i}`); in the blueprint language, they are bare references inside a `foreach` body (`elem.bucketName`, `i`). In the blueprint language the `each` field itself is written as the `foreach <expression>` inline statement.
...
<Callout title="Tip" type="info">
`each` does not accept a value that resolves to an object or mapping. To feed an object's values into `each`, unwrap them with `vals`:
...
```blueprintlang
foreach vals(values.bucketsConfig)
```
</Callout>
...
```blueprintlang
resource s3Buckets: aws/s3/bucket {
    ...
    spec {
        ...
        tags = [
            { key = "bucketNumber", value = "bucket-${i}" }
        ]
    }
}
```
...
<FieldHeading fieldName="linkSelector" />
...
<Callout title="Note" type="info">
A `linkSelector` is one of two ways to activate a link. For some resource fields, a reference to another resource activates the link directly, with no `linkSelector` required. See [links](/docs/bluelink/blueprints/links) for details.
</Callout>
...
<FieldHeading fieldName="removalPolicy" />

Controls what happens to the underlying resource in the provider when the resource is removed from the blueprint (either by being deleted from the source file or by a `condition` that evaluates to `false` on a subsequent deployment).

The field takes one of two literal string values:

- `delete` (default) - the underlying resource is destroyed in the provider when it is removed from the blueprint.
- `retain` - the underlying resource is left untouched in the provider and simply removed from the blueprint's managed state. Implementations of the spec should still clean up any internal bookkeeping (links, drift tracking, state entries) associated with the resource.

<Callout title="Tip" type="info">
Use `retain` for stateful resources that you want to protect from accidental destruction. For example, databases, object storage buckets with data, or resources you are handing off to another management system.
</Callout>

The value must be a static literal. Substitutions (`${..}`) are **not** supported for this field, because lifecycle decisions should be fixed at authoring time rather than depending on runtime variable state.

An example of using `removalPolicy` for a resource would be:
...
<Callout title="Caution" type="warning">
When `removalPolicy: retain` is set, the resource will be orphaned in the provider when removed from the blueprint. Implementations should surface a clear log or change-staging message when a retained resource is detached, so operators are aware that a resource is no longer managed but still exists.
</Callout>

**field type** 

string (one of `delete`, `retain`)

___

<FieldHeading fieldName="spec (required)" />
...
between resources or can be used for standalone metadata for a resource.

_All values that are properties of the resource in the upstream provider should be a part
of the resource spec._

Annotations should be well documented by providers for resources and their links
to allow for easy discovery of the available options and required context needed for a resource or link to be effective.

The value of an annotation must be a scalar, when `${..}` substitutions are used, the value must resolve to scalar values.
...
resource ordersSecrets: aws/secretsmanager/secret {
    metadata {
...
resource getOrdersFunction: aws/lambda/function {
    metadata {
...
<FieldHeading fieldName="byLabel" />
...
<FieldHeading fieldName="exclude" />

An optional list of resource names to exclude from the link selection.
This provides a way to handle disambiguation when multiple resources match the `byLabel` criteria
but only specific resources should be linked.

When a resource name is included in the `exclude` list, it will not be linked to even if its labels
match the `byLabel` selector criteria.

<Callout title="Note" type="info">
Substitutions are not allowed in exclude values. The values must be literal resource names as defined in the blueprint.
</Callout>
...
```blueprintlang
# Links to all resources with app = orders EXCEPT testOrdersTable
resource saveOrderFunction: aws/lambda/function {
    select by label {
        app = "orders"
...
# This lambda excludes a different resource
resource sendOrderNotification: aws/lambda/function {
    select by label {
        app = "orders"
        exclude = [ordersTable]
    }
...
# Target resources, both have the app = orders label
resource ordersTable: aws/dynamodb/table {
    metadata {
        labels = {
            app = "orders"
...
resource testOrdersTable: aws/dynamodb/table {
    metadata {
        labels = {
            app = "orders"
        }
...
In the example above, `saveOrderFunction` will link to `ordersTable` but not `testOrdersTable`,
while `sendOrderNotification` will link to `testOrdersTable` but not `ordersTable`.
...
<FieldHeading fieldName="path (required)" />
...
```blueprintlang
# main-blueprint.bp
include child "child-blueprint.yaml" {}
```
...
```blueprintlang
# app_infra/main-blueprint.bp
include child "${cwd()}/core_infra/child-blueprint.yaml" {}
```
...
```blueprintlang
# main-blueprint.bp
include coreInfrastructure "core-infra-2025-11-02.yaml" {
    metadata {
        sourceType = "aws/s3"
        bucket = "order-system-blueprints"
        region = "eu-west-1"
...
include:
  coreInfrastructure:
    path: core-infra-2025-11-02.yaml
    metadata:
      sourceType: aws/s3
      bucket: order-system-blueprints
      region: eu-west-1
...
<FieldHeading fieldName="variables" />
...
<NonLinkableHeading>Equality Operators ( = | != )</NonLinkableHeading>
...
<NonLinkableHeading>Comparative Operators ( `>` | `>=` | `<` | `>=` )</NonLinkableHeading>
...
<NonLinkableHeading>In Operators ( in | not in )</NonLinkableHeading>
...
<NonLinkableHeading>Has Key Operators ( has key | not has key )</NonLinkableHeading>
...
<NonLinkableHeading>Contains Operators ( contains | not contains )</NonLinkableHeading>
...
<NonLinkableHeading>Starts With Operators ( starts with | not starts with )</NonLinkableHeading>

The same rules apply to the `starts with` and `not starts with` operators, `not starts with` will be the negation of the behaviour for the comparisons described below.
...
<NonLinkableHeading>Ends With Operators ( ends with | not ends with )</NonLinkableHeading>
...
This specification supports YAML, JWCC and a first-class configuration language for blueprints tailored towards authoring blueprints. All three formats map to the same canonical schema, so a blueprint authored in any of them is semantically equivalent in the others. Where YAML and JWCC embed the substitution language inside `${..}` strings, the blueprint language treats expressions as first-class: references, function calls, and infix operators (`==`, `!=`, `<`, `<=`, `>`, `>=`, `&&`, `||`, `!`) are written directly as field values, and `${..}` is reserved for interpolation inside a string literal.

[Blueprint Language Spec](/docs/bluelink/blueprints/language)

<br/>

## References & Substitutions

In a blueprint, references are ways to access values from a number of different types of objects in a blueprint. References can be used to make use of variables, values, resource fields, data source fields, and exported fields from child blueprints.

In YAML and JWCC, references are wrapped in the `${..}` substitution syntax, which can also apply functions to a value. The one exception is the export `field` attribute, which the canonical schema treats as a plain string path; it is written without `${..}` and accepts only a reference path, not a function call.

In the blueprint language, references are bare expressions and appear directly as field values (for example, `timeout = variables.timeout` and `value = list(...)`). `${..}` is still available, but only for interpolating an expression into a string literal (`"https://${variables.host}/api"`). The same export `field` restriction applies: it accepts a bare reference path, not a computed expression.

The reference forms and accessors documented below are identical across all three formats; only the wrapping differs.
...
<NonLinkableHeading>Variable Reference Format</NonLinkableHeading>
...
quoted name            =   ( "'" , name in quotes , "'" ) | ( '"' , name in quotes , '"' ) ;
...
<NonLinkableHeading>Value Reference Format</NonLinkableHeading>
...
can be referenced using either double or single quotes:
...
<NonLinkableHeading>Resource Reference Format</NonLinkableHeading>
...
<NonLinkableHeading>Data Source Reference Format</NonLinkableHeading>
...
<NonLinkableHeading>Element Reference Format</NonLinkableHeading>
...
<NonLinkableHeading>Child Blueprint Reference Format</NonLinkableHeading>
...
There are a set of utility functions that can be used wherever an expression is allowed. In YAML and JWCC, functions are called inside a `${..}` substitution (for example, `${trim(variables.name)}`); in the blueprint language, the same calls are written bare as part of an expression (`trim(variables.name)`). Both forms produce the same canonical schema.

<Callout title="Caution" type="warning">
In YAML and JWCC, functions are only accepted inside `${..}` substitutions; they are not allowed in plain string values outside a substitution. In the blueprint language, function calls are bare expressions and can appear directly as field values, inside object and array literals, or interpolated into a string with `${..}`. The export `field` attribute is the one position that does not accept a function call in any format, because the canonical schema treats it as a pure path (see the [export specification](#exportdefinition)).
</Callout>
...
The following section contains the definitions of the core utility functions that should be supported by all implementations of the spec.
...
[Core Function Definitions](/docs/bluelink/blueprints/core-functions)
...
In YAML and JWCC, dynamic value substitution is performed with the `${..}` syntax, either as a standalone string (`${cacheCluster.spec.config.host}`) or as part of a string interpolation (`https://${cacheCluster.spec.config.host}:3000`). In the blueprint language, a reference that occupies the entire value is written bare (`cacheCluster.spec.config.host`), and `${..}` is reserved for interpolating an expression into a string literal (`"https://${cacheCluster.spec.config.host}:3000"`). All three formats resolve to the same canonical schema.

When a reference yields a complex type, string interpolation is not particularly useful and can lead to unexpected results. For this reason, implementations of the spec should explicitly report informative errors to the user when string interpolation is attempted on a reference that yields a complex type, regardless of which format the blueprint was authored in.

### Binary Data Handling

The Blueprint specification supports handling binary data through functions in the substitution language, enabling scenarios that require certificates, keys, code packages, and other binary content.

#### Binary Data Functions

The substitution language provides core functions for working with binary data. For detailed information about these functions, see the [Core Function Definitions](/docs/bluelink/blueprints/core-functions).

The core functions for binary data include:
- `file(path)` - Reads binary data from a file path (local or remote)
- `base64encode(data)` - Encodes binary data to base64 string
- `base64decode(string)` - Decodes base64 string to binary data
- `sha256(data)` - Computes SHA-256 hash of input data
- `md5(data)` - Computes MD5 hash of input data
- `sha1(data)` - Computes SHA-1 hash of input data
...
```blueprintlang
# Local files
resource certificate: aws/acm/certificate {
    spec {
        certificateBody = base64encode(file("certificates/server.pem"))
        privateKey = base64encode(file("certificates/private.key"))
    }
}
...
resource certificate: aws/acm/certificate {
    spec {
        certificateBody = base64encode(file("s3://my-bucket/certificates/server.pem"))
        privateKey = base64encode(file("s3://my-bucket/certificates/private.key"))
    }
}
```
...
resource function: aws/lambda/function {
    spec {
        code = {
            zipFile = base64encode(file("dist/function.zip"))
        }
    }
}
...
resource function: aws/lambda/function {
    spec {
        code = {
            zipFile = base64encode(file("s3://my-bucket/dist/function.zip"))
        }
    }
}
```
...
```blueprintlang
data certificate: aws/acm/certificate {
    export certificateBody: string
}

# Process binary data before storing
value certificateHash: string {
    value = sha256(base64decode(datasources.certificate.certificateBody))
}

resource application: aws/ec2/instance {
    spec {
        # Use the processed certificate hash
        certificateHash = values.certificateHash
    }
}
```
...
#### Encoding Behavior for Blueprint Element Fields

When a substitution resolves to a field from a blueprint element (resource spec, data source export, etc.), the field value will always be resolved as a **UTF-8 encoded string** by default.

**Examples:**

```yaml
# Resource spec fields resolve to UTF-8 strings
${resources.database.spec.endpoint}        # UTF-8 string
${resources.function.spec.functionName}   # UTF-8 string

# Data source exports resolve to UTF-8 strings  
${datasources.network.vpc}                # UTF-8 string
${datasources.certificate.certificateBody} # UTF-8 string (base64-encoded content)

# Child blueprint exports resolve to UTF-8 strings
${children.coreInfra.databaseHost}        # UTF-8 string
```

**Binary Data Handling:**
- Binary data from `file()` function is returned as raw bytes
- Use `base64encode()` to convert binary data to base64 strings for storage/transmission
- Use `base64decode()` to convert base64 strings back to binary data for processing
- All blueprint element fields are UTF-8 strings, even when they contain base64-encoded binary content
...
<Callout title="Invalid" type="error">
```yaml
resources:
...
<Callout title="Valid" type="success">
```yaml
values:
  s3BucketName:
    type: string
...
<Callout title="Not Recommended" type="warning">
```yaml
values:
  s3BucketName:
    type: string
...
References can **not** be used in the `removalPolicy` property. Lifecycle decisions must be fixed at authoring time rather than depending on runtime variable state.
...
resources:
  ordersTable:
    type: aws/dynamodb/table
    removalPolicy: ${variables.retainResources}
```
</Callout>
...
### None Values and Field Omission

The `none` literal value in the substitution language provides a way to explicitly represent the absence of a value. When a substitution evaluates to `none`, the behavior depends on the context in which it appears.

#### Field Omission at Materialisation

When a field's value evaluates to `none`, that field is **completely omitted** from the final materialised blueprint output:
...
resource instance: aws/ec2/instance {
    spec {
        ...
        monitoring = if(variables.env == "prod", true, none)
    }
}
```
...
When `variables.env` is `"dev"`, the materialised output omits the `monitoring` field entirely:
...
resources:
  instance:
    ...
    spec:
      instanceType: t3.micro
      # monitoring field is not present
```
</Callout>

This is fundamentally different from setting a field to an empty value like `""` or `false`, which would include the field with that empty value.

#### None in String Interpolation

When `none` appears within a string interpolation expression, it is **treated as an empty string** (`""`):

<Callout title="Info" type="info">
Blueprint Language

```blueprintlang
resource api: aws/apigateway/restapi {
    spec {
        # If customDomain is none, it becomes an empty string in the interpolation
...
If `customDomain` evaluates to `none`, the result is `"https:///api/v1"` (note the double slashes).
</Callout>

The field is always present for string interpolation, and `none` values simply contribute nothing to the string.

If you want to provide a default value when a variable might be `none`, use the `or()` function:
...
resource api: aws/apigateway/restapi {
    spec {
        # If customDomain is none, use "localhost" instead
        url = "https://${or(customDomain, "localhost")}/api/v1"
    }
}
```
...
Result when `customDomain` is `none`: `"https://localhost/api/v1"`
</Callout>

#### None in Arrays

When `none` values appear as elements in array literals or array-valued substitutions, they are **filtered out** from the final array:
...
resource securityGroup: aws/ec2/securitygroup {
    spec {
        ingressRules = [
            ...
            if(variables.env == "dev", { port = 22, protocol = "tcp" }, none),
  ...
  securityGroup:
    ...
    spec:
      ingressRules:
        - port: 443
          protocol: tcp
        - ${if(eq(variables.env, "dev"), object(port=22, protocol="tcp"), none)}
        - port: 80
          protocol: tcp
```

When `variables.env` is `"prod"`, the `none` element is filtered out, resulting in an array with 2 rules instead of 3.

#### None in Objects

When `none` values appear as values in object/mapping literals, the corresponding key-value pair is **omitted** from the final object:

Blueprint Language

```blueprintlang
resource lambda: aws/lambda/function {
    spec {
        functionName = "myFunction"
        environment = {
            NODE_ENV = variables.nodeEnv,
            DEBUG_MODE = if(variables.env == "dev", "true", none),
            LOG_LEVEL = "info"
        }
    }
}
...
resources:
  lambda:
    ...
    spec:
      ...
      environment:
        NODE_ENV: ${variables.nodeEnv}
        DEBUG_MODE: ${if(eq(variables.env, "dev"), "true", none)}
        LOG_LEVEL: info
```

When `variables.env` is `"prod"`, the `DEBUG_MODE` key is omitted from the `environment` object.
...
```blueprintlang
# Only include tags in production
resource bucket: aws/s3/bucket {
    spec {
        bucketName = "my-bucket"
        tags = if(variables.env == "prod", values.prodTags, none)
    }
}
```
...
resources:
  bucket:
    ...
    spec:
      bucketName: my-bucket
      tags: ${if(eq(variables.env, "prod"), values.prodTags, none)}
```
...
```blueprintlang
# Provide default when value might be none
resource function: aws/lambda/function {
    spec {
        timeout = coalesce(variables.customTimeout, 30)
...
<Callout title="Tip" type="info">
**Choosing between `none` and `""` for fallback values:**

Use `none` when you want the field to be **omitted** from the output:
```yaml
# Field not present when condition is false
optionalDescription: ${if(hasDescription, description, none)}
```

Use `""` (empty string) when you want the field to be **present but empty**:
```yaml
# Field present with empty value when condition is false
description: ${if(hasDescription, description, "")}
```

**Rule of thumb:** If the field's absence vs. presence matters to the consuming system, use `none` for conditional omission. If the field should always be present, use `""` or another appropriate empty value.
</Callout>

<Callout title="Tip" type="info">
**Remember: `none` becomes empty string in interpolation:**

When `none` appears in string interpolation, it is treated as `""`:

```yaml
# If domain is none → "https:///api" (note the double slashes)
url: "https://${domain}/api"

# Better: provide a meaningful default with coalesce()
url: "https://${coalesce(domain, "localhost")}/api"
```

This is especially important for URLs, file paths, and other formatted strings where an empty segment might be invalid or confusing.
</Callout>

#### See Also

- [Substitution Grammar](#substitution-grammar) - Formal grammar including `none` literal
- [Core Functions: Special Values](/docs/bluelink/blueprints/core-functions#special-values) - Detailed `none` value documentation
- [Core Functions](/docs/bluelink/blueprints/core-functions) - Functions that handle `none` values
...
quoted name                  =   ( "'" , name in quotes , "'" ) | ( '"' , name in quotes , '"' ) ;
index accessor               =   "[" , [ natural number ] , "]" ;
literal                      =   bool literal | float literal | int literal | string literal | none literal ;
...
A blueprint does not necessarily need to be a single file, it can be split into multiple files and directories.
...
`core-infra.bp` (Blueprint Language)
...
`app-infra.bp` (Blueprint Language)
...
resource api: aws/api-gateway/rest-api {
...
`app-infra/main-blueprint.bp` (Blueprint Language)
...
include topics "${cwd()}/core-infra/topics.yaml" {
    variables {
        orderTopicType = variables.orderTopicType
    }
}

include eventBus "${cwd()}/core-infra/event-bus.yaml" {
    variables {
        eventBusName = variables.eventBusName
    }
}

include appInfrastructure "${cwd()}/app-infra/api.yaml" {
    variables {
...
`core-infra/topics.bp` (Blueprint Language)
...
`core-infra/event-bus.bp` (Blueprint Language)

```blueprintlang
version "2025-11-02"

variable eventBus: string {
    default = "default"
    description = "The name of the event bus to be used for order events"
}

resource eventBus: aws/eventbridge/event-bus {
...
`app-infra/api.bp` (Blueprint Language)
...
resource api: celerity/blueprint {
    description = "The API for the system"
...
resource coreInfra: celerity/blueprint {

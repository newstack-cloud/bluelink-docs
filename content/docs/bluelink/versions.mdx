...
The Bluelink CLI and AWS Provider plugin are installed as part of the **Bluelink profile** (default). The Celerity Transformer and Celerity plugin are installed as part of the **Celerity profile**, which also includes the AWS Provider. The Deploy Engine, Plugin Framework, and Blueprint Language Server are shared across both profiles. See [Profiles](/docs/bluelink/profiles) for details on how profiles affect which components are installed.
...
The cloud provider plugins (AWS, Azure, Google Cloud) are shared across both the Bluelink and Celerity profiles. The Celerity Transformer is specific to the Celerity profile. See [Profiles](/docs/bluelink/profiles) for details.

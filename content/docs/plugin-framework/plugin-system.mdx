---
title: Plugin System
description: The system for discovering and managing plugins
---
...
Out of the box, the Deploy Engine discovers plugins by looking for executables under the `$HOME/.bluelink/engine/plugins/bin` directory on Linux or macOS and `%LOCALAPPDATA%\NewStack\Bluelink\engine\plugins` on Windows.
The root directory is expected to contain a directory for each type of plugin. Provider plugins should be in the `providers` directory, and transformer plugins should be in the `transformers` directory.

Each plugin is expected to be in a subdirectory following the ID format of `{hostname/}?{namespace}/{plugin}`. 
For example, a plugin for the `bluelink/aws` provider would be located at `$HOME/.bluelink/engine/plugins/bin/providers/bluelink/aws` on Linux or macOS.
On Windows, the plugin would be located at `%LOCALAPPDATA%\NewStack\Bluelink\engine\plugins\providers\bluelink\aws`.
Another example, would be that the plugin with the ID `registry.customhost.com/bluelink/azure` would be expected to be located in `$HOME/.bluelink/engine/plugins/bin/providers/registry.customhost.com/bluelink/azure` on Linux or macOS.
The `{hostname/}` part is only required if the plugin is hosted on a custom registry, all plugins that have an ID in the form `{namespace}/{plugin}` are assumed to be hosted in the Bluelink Framework registry.

On Windows, the plugin with the full path format would be located at `%LOCALAPPDATA%\NewStack\Bluelink\engine\plugins\providers\registry.customhost.com\bluelink\azure`.

In each plugin directory, there should be a folder for each version of the plugin. The version folder should be named with the version number of the plugin. For example, the plugin version `1.0.0` would be located at `$HOME/.bluelink/engine/plugins/bin/providers/bluelink/aws/1.0.0`.

Each plugin executable is expected to be a file named `plugin` with no file extension on Linux or macOS. On Windows, the plugin executable is expected to be a file named `plugin.exe`.
An example of a full path to the plugin executable file would be `$HOME/.bluelink/engine/plugins/bin/providers/bluelink/aws/1.0.0/plugin` for Linux or macOS.
On Windows, the full path to the plugin executable file would be `%LOCALAPPDATA%\NewStack\Bluelink\engine\plugins\providers\bluelink\aws\1.0.0\plugin.exe`.

For a standard install of the Deploy Engine, the `$HOME/.bluelink/engine/plugins/bin` or `%LOCALAPPDATA%\NewStack\Bluelink\engine\plugins` directory is created for you and populated with the core plugins that enable the Bluelink resource types.

You can specify a custom directory for the Deploy Engine to look for plugins by appending a new directory to the `BLUELINK_DEPLOY_ENGINE_PLUGIN_PATH` environment variable. This environment variable is a list of directories that the Deploy Engine will search for plugins in. Directories are separated by colons (`:`) on Linux and macOS, and semicolons (`;`) on Windows. This environment variable will be set as a part of the standard Bluelink installation with a default value of `$HOME/.bluelink/engine/plugins/bin` on Linux or macOS
and `%LOCALAPPDATA%\NewStack\Bluelink\engine\plugins` on Windows.
If, for some reason, this environment variable is not set, the Deploy Engine will default to the aforementioned default values.
An example for a custom plugin path would be to set `BLUELINK_DEPLOY_ENGINE_PLUGIN_PATH` to `$HOME/.bluelink/engine/plugins/bin:/path/to/custom/plugins` on Linux or macOS
and `%LOCALAPPDATA%\NewStack\Bluelink\engine\plugins;C:\path\to\custom\plugins` on Windows.

The priority of plugin discovery is based on the order of the directories in the `BLUELINK_DEPLOY_ENGINE_PLUGIN_PATH` environment variable. The Deploy Engine will search for plugins in the directories in the order they are listed in the environment variable from left to right.
...
- `{namespace}/{plugin}` - This format is used for plugins that are hosted in the Bluelink registry.

An example for the former format would be `registry.customhost.com/bluelink/azure`.

An example for the latter format would be `bluelink/aws`.
...
When a plugin hasn't registered itself with the host after a pre-determined deadline, the Deploy Engine will kill the plugin process. It will then attempt to restart the plugin process up to a maximum of 4 times (5 launch attempts as per the default configuration). The attempt limit is configurable via the `BLUELINK_DEPLOY_ENGINE_PLUGIN_LAUNCH_ATTEMPT_LIMIT` environment variable.
...
The actual retry behaviour is managed by the [Blueprint Framework](https://bluelink.dev/docs/blueprint-framework), the Deploy Engine maps these error codes to errors that are a part of the framework's error handling system used in the data retrieval, change staging and deployment processes.
...
Plugins can depend on other plugins, this is done by specifying a `dependencies` map in the plugin's `bluelink-registry-info.json` file. Dependencies between plugins allow the pinning of specific versions of plugins to ensure compatibility between plugins loaded into the Deploy Engine (or other host).
...
Plugin IDs can be the short form `{namespace}/{plugin}` or the full form `{hostname/}?{namespace}/{plugin}`. The short form is used for plugins that are hosted in the Bluelink registry, while the full form is used for plugins that are hosted on a custom registry.

An example of a `bluelink-registry-info.json` file with dependencies would look like this:
...
<Callout title="About Dependency Locking" type="info">
...
- [Service v1 gRPC Reference](https://github.com/newstack-cloud/bluelink/tree/main/libs/deploy-engine/plugin/providerservice/service.proto)

...
description: Using the Bluelink Signature v1 for authentication with Bluelink components
---

Bluelink Signature v1 can be used to authenticate with Bluelink components that provide HTTP APIs such as the [Deploy Engine API](/docs/deploy-engine).

## Specification

Bluelink Signature v1 is a custom authentication mechanism that avoids the transportation of private API keys over the wire by using them in combination with a public key ID, timestamp and custom request headers to create a signature.

### Key Pairs

Bluelink Signature v1 uses a key pair that consists of a key ID and a secret key. The key ID is used to identify the key pair and the secret key is used to sign the message.
...
You can generate a key pair using the Bluelink CLI:

```bash
bluelink signature-v1 create-key-pair
```
...
```
{keyId},bluelink-date={timestamp},{header1-name}={header1-value},...,{headerN-name}={headerN-value}
```

`{timestamp}` is a unix timestamp in seconds.

<Callout title="Header name case" type="warning">
...
The signature is included in the `Bluelink-Signature-V1` header of a HTTP request in the following format:

```
Bluelink-Signature-V1: keyId="{keyId}", headers="bluelink-date {header1-name} ... {headerN-name}", signature="{signature}"
```
...
`bluelink-date` is a special header that is used to prevent replay attacks. The timestamp in the message should be within a certain time window of the current time on the server to prevent replay attacks.

All the headers listed and used to create the signature must be present in the HTTP request.

<Callout title="Order of parts in the header value" type="warning">
The parts of the `Bluelink-Signature-V1` header must be in the following order:
1. `keyId`
2. `headers`
3. `signature`
</Callout>
...
<Callout title="Header name case" type="warning">
All header names must be in lowercase in the `headers` part of the `Bluelink-Signature-V1` header.
</Callout>
...
1. Extract the key ID, headers and signature from the `Bluelink-Signature-V1` header of the HTTP request.
2. Check each header listed in the headers field of the signature is present in the HTTP request.
3. Create the [message](#message) using the public key, timestamp in the `Bluelink-Date` header and custom request headers.
...
6. Compare the signature created in step 5 with the signature in the `Bluelink-Signature-V1` header using a secure HMAC comparison that is resistant to timing attacks.

7. If the signatures match, check that the timestamp in the `Bluelink-Date` header is within a certain time window of the current time on the server to prevent replay attacks.
...
You can find the supported approaches in the documentation for each Bluelink component that supports Signature v1.

## Revoking Key Pairs

To revoke Signature v1 Key pairs, delete the key pair from the configuration file or environment variable and restart the Bluelink component that uses it.

## Using the Signature

To use the signature, you must include it in the `Bluelink-Signature-V1` header of a HTTP request.

```
POST /v1/run HTTP/1.1
Host: api.workflow.example.com
Bluelink-Signature-V1: keyId="{keyId}", headers="bluelink-date", signature="{signature}"
Bluelink-Date: {timestamp}

...
- `$HOME/.bluelink/clients/bluelink.config.yml` on Linux or macOS and `%LOCALAPPDATA%\NewStack\Bluelink\clients\bluelink.config.yml` on Windows.
- `$HOME/.bluelink/clients/bluelink.config.yaml` on Linux or macOS and `%LOCALAPPDATA%\NewStack\Bluelink\clients\bluelink.config.yaml` on Windows.
- `$HOME/.bluelink/clients/bluelink.config.json` on Linux or macOS and `%LOCALAPPDATA%\NewStack\Bluelink\clients\bluelink.config.json` on Windows.
- `$HOME/.bluelink/clients/bluelink.config.toml` on Linux or macOS and `%LOCALAPPDATA%\NewStack\Bluelink\clients\bluelink.config.toml` on Windows.
...
By default, the CLI will look for a configuration file at `$HOME/.bluelink/clients/engine.auth.json` on Linux or macOS and `%LOCALAPPDATA%\NewStack\Bluelink\clients\engine.auth.json` on Windows. 

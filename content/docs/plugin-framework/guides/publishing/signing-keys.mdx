...
description: A guide for creating signing keys for your plugins
---

In order to publish plugins to the Bluelink Registry, you will need to sign your plugin binaries with a GPG Signing Key using GoReleaser or similar.
...
The Bluelink Registry only has support for RSA keys, so you should select `1` to choose the `RSA and RSA` key type.
...
You will need to add the public key to your Bluelink Registry account to validate the signature for your plugin releases when users install them with the Bluelink CLI.

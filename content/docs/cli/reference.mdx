...
**Environment Variable**: `BLUELINK_CLI_SKIP_PLUGIN_CHECK`

**Configuration Key**: `skipPluginCheck`

**Type**: `boolean`

**Default Value**: `false`

Skip the automatic plugin dependency check that runs before the
`validate`, `stage`, `deploy` and `destroy` commands.

When the Deploy Engine is running on the same machine as the CLI
(connected via unix socket or TCP to localhost), the CLI automatically
checks the deploy configuration file for plugin dependencies and installs
any that are missing. Set this flag to disable that behaviour.
...
This can be `yaml`, `jsonc`, or `bp` (the blueprint language).
Selecting `bp` scaffolds the project with a `project.bp` file.
...
This is mostly useful in interactive mode where you can navigate and filter
the templates available. The interactive mode displays a split pane with a
list of templates on the left and template details on the right.

#### `--search`

**Environment Variable**: `BLUELINK_CLI_TEMPLATES_LIST_SEARCH`

**Configuration Key**: `templatesListSearch`

**Type**: `string`

**Default Value**: `""`

Filter templates by name using a case-insensitive substring match.
...
The blueprint language (`.bp`/`.blueprint`) is supported alongside the YAML (`.yaml`/`.yml`)
and JSON with Comments (`.jsonc`/`.json`) formats.
When this flag is not set, an existing `project.blueprint.*` or `project.bp` file in the
current directory is auto-detected, falling back to `project.blueprint.yaml`.
...
**Environment Variable**: `BLUELINK_CLI_VALIDATE_TRANSFORM_SPEC`
...
**Environment Variable**: `BLUELINK_CLI_VALIDATE_AFTER_TRANSFORM`
...
This option is only relevant for existing blueprint instances. It will only be used if the `--instance-id` or `--instance-name` flags are provided. In interactive mode (when neither flag is provided), the destroy prompt will only appear if you enter an instance name that corresponds to an existing deployment.

### `--skip-drift-check`

**Environment Variable**: `BLUELINK_CLI_STAGE_CHANGES_SKIP_DRIFT_CHECK`

**Configuration Key**: `stageChangesSkipDriftCheck`

**Type**: `boolean`

**Default Value**: `false`

If true, drift checks will be skipped during the change staging process.
Drift checks compare the current state of deployed resources against the expected state
to detect any out-of-band changes made outside of the deploy engine.

Skipping drift checks can be useful when you are certain no external changes have been made
or when you want to speed up the staging process for large deployments.

This option is only relevant for existing blueprint instances. It will only be used if the `--instance-id` or `--instance-name` flags are provided. In interactive mode (when neither flag is provided), the skip drift check prompt will only appear if you enter an instance name that corresponds to an existing deployment.

### `--json`

**Environment Variable**: `BLUELINK_CLI_STAGE_JSON`

**Configuration Key**: `stageJson`

**Type**: `boolean`

**Default Value**: `false`

Output the result as a single JSON object when the operation completes.
This is useful for CI/CD pipelines and programmatic tooling consumption.

When enabled, JSON mode implies non-interactive mode (no TUI, no streaming text output).
The output includes the change set ID, instance information, the full set of changes, and a summary of the changes.

If drift is detected during staging, the JSON output will include drift information and reconciliation details.

See the [JSON Output Reference](/docs/cli/json-output#stage-command) for detailed examples of the JSON output format.
...
When `--json` is enabled, `--auto-approve` is implied and does not need to be explicitly set.
...
When used in combination with `--stage`, this flag will also skip drift checks during the change staging process.

<Callout title="Caution" type="error">
Use this flag with care. Only use this when you are certain the instance is in a stuck state and no other deployment process is _actively_ running.
</Callout>

<Callout title="Note" type="info">
This option is only available as a command-line flag, environment variable, or configuration file setting. There is no interactive prompt for this option.
</Callout>
...
**Environment Variable**: `BLUELINK_CLI_DEPLOY_JSON`

**Configuration Key**: `deployJson`
...
When enabled, JSON mode implies non-interactive mode (no TUI, no streaming text output).
The output includes the full blueprint instance state along with additional metadata.
On failure, the output includes failure reasons and rollback status if applicable.

See the [JSON Output Reference](/docs/cli/json-output#deploy-command) for detailed examples of the JSON output format.
...
**Environment Variable**: `BLUELINK_CLI_DESTROY_JSON`

**Configuration Key**: `destroyJson`
...
When enabled, JSON mode implies non-interactive mode (no TUI, no streaming text output).
The output includes the destroy operation status and summary details.
On failure, the output includes failure reasons.

See the [JSON Output Reference](/docs/cli/json-output#destroy-command) for detailed examples of the JSON output format.

## `instances`

This is the command category for managing and viewing blueprint instances.

### `list`

`bluelink instances list [options]`

This command lists blueprint instances managed by the deploy engine.
By default, all instances are listed. In interactive mode, the list is paginated
and you can use the search functionality to filter instances.
...
```bash
bluelink instances list --search "order"
```
...
**Environment Variable**: `BLUELINK_CLI_INSTANCES_LIST_SEARCH`

**Configuration Key**: `instancesListSearch`

**Type**: `string`

A search term to filter instances by name. The search is case-insensitive and matches
instances whose names contain the search term.

In interactive mode, you can also use the built-in search functionality to filter
the paginated list of instances.

#### `--json`

**Environment Variable**: `BLUELINK_CLI_INSTANCES_LIST_JSON`

**Configuration Key**: `instancesListJson`
...
When enabled, JSON mode implies non-interactive mode.
The output includes instance IDs, names, status, and last deployment timestamps.

See the [Instances List Command](/docs/cli/json-output#instances-list-command) section of the JSON Output Reference for detailed output examples.

### `inspect`

`bluelink instances inspect [options]`
...
```bash
bluelink instances inspect --instance-name order-stack
```

Inspecting a pre-selected instance by ID:

```bash
bluelink instances inspect --instance-id a471bbb8-1267-4e65-873b-fa86bca71fcf
```

Inspecting a pre-selected instance as JSON:

```bash
bluelink instances inspect --json --instance-name order-stack
```

#### `--instance-name`

**Environment Variable**: `BLUELINK_CLI_INSTANCES_INSPECT_INSTANCE_NAME`

**Configuration Key**: `instancesInspectInstanceName`
...
**Environment Variable**: `BLUELINK_CLI_INSTANCES_INSPECT_INSTANCE_ID`

**Configuration Key**: `instancesInspectInstanceId`
...
**Environment Variable**: `BLUELINK_CLI_INSTANCES_INSPECT_JSON`

**Configuration Key**: `instancesInspectJson`
...
When enabled, JSON mode implies non-interactive mode.
The output includes the full blueprint instance state with resources, links, child blueprints, exports and deployment status.

See the [Instances Inspect Command](/docs/cli/json-output#instances-inspect-command) section of the JSON Output Reference for detailed output examples.

## `state`

This is the command category for managing deploy engine state between different environments.
These commands use a backend-agnostic JSON format, allowing state to be shared between environments
regardless of the storage backend (memfile or postgres).

An example use case would be a CI/CD pipeline that runs the `stage` and `deploy` commands for a blueprint deployment.
Through the use of these commands, the pipeline can import the deploy engine state from the previous run of the pipeline and carry out the next deployment to the same environment.

### `--engine-config-file`

**Environment Variable**: `BLUELINK_CLI_STATE_ENGINE_CONFIG_FILE`

**Configuration Key**: `stateEngineConfigFile`
...
The path to the deploy engine configuration file. This is required to determine how state data
is currently stored so that import and export operations can read from or write to the correct storage backend.

### `import`

`bluelink state import [options]`

This command is used to import blueprint instances from a local file or a remote source.
The import uses a backend-agnostic JSON format, allowing state to be migrated between different
storage backends (e.g., from memfile to postgres or vice versa).

<Callout title="Caution" type="warning">
When importing state, existing instances with matching IDs will be overwritten with the imported state.
</Callout>

Run the command without any flags and go through the interactive prompts to import state:

```bash
bluelink state import
```

Importing state from a local file:
```bash
bluelink state import --file ./deployment-state.json
```

Importing state from a remote source:
```bash
bluelink state import --file s3://bucket-name/path/to/state-export.json
```

#### `--file`

**Environment Variable**: `BLUELINK_CLI_STATE_IMPORT_FILE`

**Configuration Key**: `stateImportFile`

**Type**: `string`

The path to the input file to import.
The input file should be a JSON file containing an array of blueprint deployment instances,
as exported by the `state export` command.
...
**Environment Variable**: `BLUELINK_CLI_STATE_IMPORT_JSON`

**Configuration Key**: `stateImportJson`
...
Output the result as a single JSON object.
This is useful for CI/CD pipelines and programmatic tooling consumption for importing state.

See the [State Import Command](/docs/cli/json-output#state-import-command) section of the JSON Output Reference for detailed output examples.

### `export`

`bluelink state export [options]`

This command is used to export the entire state or specific blueprint instances to be shared between environments.
The export uses a backend-agnostic JSON format, allowing state to be migrated between different
storage backends (e.g., from memfile to postgres or vice versa).

Run the command without any flags and go through the interactive prompts to export state:

```bash
bluelink state export
```

Exporting the entire state to a local file:
```bash
bluelink state export --file ./deployment-state.json
```

Exporting one or more specific blueprint deployment instances to a remote source:
```bash
bluelink state export --instances stack-1,stack-2,stack-3 --file s3://bucket-name/path/to/instances-export.json
```

#### `--instances`

**Environment Variable**: `BLUELINK_CLI_STATE_EXPORT_INSTANCES`

**Configuration Key**: `stateExportInstances`
...
When this option is not set, all instances will be exported (full state export).

#### `--file`

**Environment Variable**: `BLUELINK_CLI_STATE_EXPORT_FILE`

**Configuration Key**: `stateExportFile`

**Type**: `string`

The path to the output file to write the export to.
The output file will be a JSON file containing an array of blueprint deployment instances.
...
**Environment Variable**: `BLUELINK_CLI_STATE_EXPORT_JSON`

**Configuration Key**: `stateExportJson`
...
Output the result as a single JSON object.
This is useful for CI/CD pipelines and programmatic tooling consumption for exporting state.

See the [State Export Command](/docs/cli/json-output#state-export-command) section of the JSON Output Reference for detailed output examples.

## `cleanup`

`bluelink cleanup [options]`

This command triggers cleanup of temporary resources in the deploy engine that have exceeded their configured retention periods.
The deploy engine stores temporary data such as validation results, change sets, reconciliation results, and streaming events
for a configurable period to support operations like event replay and debugging.

Running cleanup manually can be useful to free up storage space or to ensure old data is removed
before a scheduled cleanup would normally occur.

<Callout title="Remote Deploy Engine Support" type="info">
Unlike commands such as `state import`, `state export`, and `plugins` subcommands that only work
with a local deploy engine instance, the `cleanup` command works with both local and remote
deploy engine instances. For remote instances, ensure the correct endpoint is set and that the correct credentials are configured
in the [engine auth config file](/docs/cli/reference#--engine-auth-config-file).
</Callout>

Trigger cleanup of all supported resource types (default behavior):

```bash
bluelink cleanup
```

Trigger cleanup of specific resource types:
...
**Environment Variable**: `BLUELINK_CLI_CLEANUP_VALIDATIONS`

**Configuration Key**: `cleanupValidations`
...
Validation results are created when running the `validate` command and are retained
for debugging and auditing purposes.

When no specific resource type flags are provided, all resource types are cleaned up by default.

### `--changesets`

**Environment Variable**: `BLUELINK_CLI_CLEANUP_CHANGESETS`

**Configuration Key**: `cleanupChangesets`
...
Change sets are created during the `stage` command and contain the planned changes
for a deployment. Old change sets that were never deployed or have already been applied
are cleaned up.
...
**Environment Variable**: `BLUELINK_CLI_CLEANUP_RECONCILIATION_RESULTS`

**Configuration Key**: `cleanupReconciliationResults`
...
Reconciliation results are created when checking for drift or interrupted state
and are retained for review and debugging purposes.

When no specific resource type flags are provided, all resource types are cleaned up by default.

### `--events`

**Environment Variable**: `BLUELINK_CLI_CLEANUP_EVENTS`

**Configuration Key**: `cleanupEvents`
...
Events are generated during validation, change staging, and deployment operations
and are retained to support event replay via the `Last-Event-ID` header for SSE streams.

When no specific resource type flags are provided, all resource types are cleaned up by default.

### Scheduled Cleanup

The deploy engine does not automatically clean up old data on a schedule. It is recommended that you set up
a scheduled task (such as a cron job) to run the cleanup command at regular intervals.

**Recommended frequency:**
- **High-usage environments** (multiple deployments per day): Weekly cleanup
- **Moderate-usage environments** (a few deployments per week): Monthly cleanup
- **Low-usage environments** (occasional deployments): Quarterly cleanup or as needed

If cleanup is not run regularly, storage usage will grow over time but there is no functional impact
on the deploy engine. Old data simply accumulates until explicitly cleaned up.

**Example cron entries:**

Weekly cleanup (every Sunday at 2:00 AM):
```bash
0 2 * * 0 /usr/local/bin/bluelink cleanup
```

Monthly cleanup (first day of each month at 3:00 AM):
```bash
0 3 1 * * /usr/local/bin/bluelink cleanup
```

<Callout title="Tip" type="info">
Adjust the path to the `bluelink` binary based on your installation. You can find the path by running
`which bluelink` on Linux/macOS or `where bluelink` on Windows.
</Callout>
...
Each plugin will be rendered with its dependency tree.
Typically, transformer plugins will depend on provider plugins
to be able to deploy the concrete resources that the transformer produces.

In interactive mode, you can navigate the plugin list and search by pressing `/`.
...
**Environment Variable**: `BLUELINK_CLI_PLUGINS_LIST_SEARCH`

**Configuration Key**: `pluginsListSearch`

**Type**: `string`

**Default Value**: `""`

Filter plugins by name using a case-insensitive substring match.
In interactive mode, you can also press `/` to enter search mode.

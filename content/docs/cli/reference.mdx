...
Use this flag with care. Only use this when you are certain the instance is in a stuck state and no other deployment process is running.
</Callout>
...
When enabled, JSON mode implies non-interactive mode (no TUI, no streaming text output).
The output includes the full blueprint instance state with resource IDs, exports, child blueprints, and deployment status.

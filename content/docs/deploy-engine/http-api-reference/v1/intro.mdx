...
description: Introduction to the Deploy Engine HTTP API reference
---

<VersionBadge version={"1.0"} />


`v1` of the Deploy Engine HTTP API provides a set of endpoints for interacting with the Deploy Engine to validate and deploy blueprints.
...
on the same host as the Deploy Engine (such as the Bluelink CLI). To provide a service to end users, such as a Platform-as-a-Service offering, the Deploy Engine
...
Unix domain sockets are useful for instances of the Deploy Engine that are running on the same host as the client making the request, such as the default installation of Bluelink where the Deploy Engine is running as a local process that the CLI communicates with.
...
```
https://api.deploy.bluelink.dev/v1
```
...
The Deploy Engine HTTP API supports authentication using API keys, JWTs and the Bluelink Signature v1. The authentication mechanism to use when making requests will depend on how the Deploy Engine is configured.
...
2. **Bluelink Signature v1**: If the `Bluelink-Signature-V1` header is present, using the Bluelink Signature v1 format, the signature will be verified and used to authenticate the request. This will be skipped if the Deploy Engine instance is not configured with at least one key pair for the Bluelink Signature v1 authentication mechanism.
3. **API Key**: If the `Bluelink-Api-Key` header is present, using the API key scheme and contains a valid API key, the API key will be used to authenticate the request. This will be skipped if the Deploy Engine instance is not configured with at least one API key for the API key authentication mechanism.

See the [Authentication](/docs/bluelink/auth) documentation to read more about the authentication mechanisms supported by the Deploy Engine.
